...
def create_vector_query(query_string, size):
    query_vectors = model.encode([query_string]) 
    query_obj = {
        'size': size,
        "query": {
            "knn": {
...
def search(client, user_query, index="bbuy_products", sort="_score", sortDir="desc", use_vector_query=False):
...
if __name__ == "__main__":
    ...
    general.add_argument('--vector', action='store_true',
                         help='If set vector query is used instead of text-based query.')                      
